...
const en = {
  ...
  oss: {
    ...
    repos: [
      ...
      { name: 'Vysp3r/ProtonPlus', desc: 'Diagnosed a Steam Linux Runtime 4.0 install-pipeline fault where Steam marks the download complete with buildid 0, so the manifest and files are never written to disk, breaking the compatibility tool for any GE-Proton launch; published a console workaround that manually pulls the depot and hand-builds an appmanifest.acf to restore the runtime without a full reinstall.', tag: 'debugging', isKind: true, action: 'Issue #1232', href: 'https://github.com/Vysp3r/ProtonPlus/issues/1232', isIssue: true },
      ...
      { name: 'Recol/DLSS-Updater', desc: 'Added DLSS/FSR/XeSS/RR version-label chips to game cards; after maintainer review flagged that hardcoded client-side version tables would silently mislabel future DLL renumbering, redesigned the feature to source labels from bounded, per-DLL ranges in the existing DLL manifest, display-only, with unmatched versions falling back to the raw string rather than a guess.', tag: 'Python', action: 'PR #283', href: 'https://github.com/Recol/DLSS-Updater/pull/283' },
...
const it: Translations = {
  ...
  oss: {
    ...
    repos: [
      ...
      { name: 'Vysp3r/ProtonPlus', desc: 'Diagnosticato un problema nella pipeline di installazione di Steam Linux Runtime 4.0: Steam segna il download come completato con buildid 0, per cui il manifest e i file non vengono mai scritti su disco, rompendo il compatibility tool per qualsiasi avvio di GE-Proton; pubblicata una soluzione via console basata sul download manuale del depot e su un appmanifest.acf costruito a mano per ripristinare il runtime senza reinstallazione completa.', tag: 'debug', isKind: true, action: 'Issue #1232', href: 'https://github.com/Vysp3r/ProtonPlus/issues/1232', isIssue: true },
      ...
      { name: 'Recol/DLSS-Updater', desc: 'Aggiunti chip con l’etichetta di versione DLSS/FSR/XeSS/RR alle card di gioco; dopo che la review del maintainer ha segnalato che tabelle di versione hardcoded lato client avrebbero potuto etichettare erroneamente future rinumerazioni delle DLL, la funzionalità è stata riprogettata per ricavare le etichette da intervalli per-DLL limitati già presenti nel manifest DLL, solo a scopo di visualizzazione, con fallback alla stringa grezza per le versioni non riconosciute invece di un’ipotesi.', tag: 'Python', action: 'PR #283', href: 'https://github.com/Recol/DLSS-Updater/pull/283' },
...
const de: Translations = {
  ...
  oss: {
    ...
    repos: [
      ...
      { name: 'Vysp3r/ProtonPlus', desc: 'Einen Installationsfehler in der Steam Linux Runtime 4.0 diagnostiziert: Steam markiert den Download mit buildid 0 als abgeschlossen, wodurch Manifest und Dateien nie auf die Festplatte geschrieben werden und das Compatibility-Tool bei jedem GE-Proton-Start kaputtgeht; einen Konsolen-Workaround veröffentlicht, der den Depot manuell herunterlädt und eine appmanifest.acf von Hand erstellt, um das Runtime ohne komplette Neuinstallation wiederherzustellen.', tag: 'Debugging', isKind: true, action: 'Issue #1232', href: 'https://github.com/Vysp3r/ProtonPlus/issues/1232', isIssue: true },
      ...
      { name: 'Recol/DLSS-Updater', desc: 'DLSS/FSR/XeSS/RR-Versions-Label-Chips zu den Spielkarten hinzugefügt; nachdem die Maintainer-Review darauf hinwies, dass hartkodierte clientseitige Versionstabellen künftige DLL-Neunummerierungen stillschweigend falsch beschriften würden, das Feature so umgestaltet, dass Labels aus begrenzten, DLL-spezifischen Bereichen im bestehenden DLL-Manifest stammen, rein zur Anzeige, wobei nicht zugeordnete Versionen auf den Rohstring statt auf eine Vermutung zurückfallen.', tag: 'Python', action: 'PR #283', href: 'https://github.com/Recol/DLSS-Updater/pull/283' },
...
const fr: Translations = {
  ...
  oss: {
    ...
    repos: [
      ...
      { name: 'Vysp3r/ProtonPlus', desc: 'Diagnostic d’une panne dans le pipeline d’installation de Steam Linux Runtime 4.0 : Steam marque le téléchargement comme terminé avec buildid 0, si bien que le manifeste et les fichiers ne sont jamais écrits sur le disque, cassant l’outil de compatibilité pour tout lancement de GE-Proton ; publication d’une solution via console basée sur un téléchargement manuel du dépôt et un appmanifest.acf construit à la main pour restaurer le runtime sans réinstallation complète.', tag: 'débogage', isKind: true, action: 'Issue #1232', href: 'https://github.com/Vysp3r/ProtonPlus/issues/1232', isIssue: true },
      ...
      { name: 'Recol/DLSS-Updater', desc: 'Ajout de puces d’étiquette de version DLSS/FSR/XeSS/RR sur les cartes de jeu ; après que la revue du mainteneur a signalé que des tables de version codées en dur côté client risquaient de mal étiqueter silencieusement de futures renumérotations de DLL, la fonctionnalité a été repensée pour puiser les étiquettes dans des plages bornées par DLL déjà présentes dans le manifeste DLL, en lecture seule, les versions non reconnues retombant sur la chaîne brute plutôt que sur une supposition.', tag: 'Python', action: 'PR #283', href: 'https://github.com/Recol/DLSS-Updater/pull/283' },
