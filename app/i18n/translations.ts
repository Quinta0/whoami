...
const en = {
  ...
  about: {
    ...
    p3: 'On the academic side I’m deepening my understanding of <strong>microeconomics, macroeconomics, game theory, and quantitative finance</strong> — and finding more overlap with systems thinking than you’d expect.',
    card: [
  ...
  desktop: {
    ...
    intro: 'Daily driver and dev machine. Built around AMD’s 3D V-Cache architecture for a blend of high single-threaded performance and serious GPU compute — doubles as a gaming rig, 3D modelling workstation, and 3D printing controller.',
  ...
  projects: {
    ...
    items: [
      ...
      { type: 'Personal Project', status: 'oss', title: 'Riptune — Terminal Music Client', desc: 'Terminal-native music client for Subsonic-compatible servers (Navidrome, Airsonic, Gonic), built in Rust for the niri Wayland compositor — MPRIS2 D-Bus control, SQLite-cached library for instant startup, and inline album art via Kitty/Sixel graphics.' },
      ...
      { type: 'Personal Project', status: 'oss', title: 're-soulcommand', desc: 'Music recommendation downloader integrating ListenBrainz and Last.fm playlists with Navidrome via slskd (Soulseek daemon). Fork rebuilt around a REST API backend — stripped of bloat, Docker-native.' },
      ...
      { type: 'Personal Project', status: 'oss', statusExtra: ' · ★ 26', title: 'ObsidianSetup', desc: 'Cross-platform utility script that makes installing Obsidian and its plugins consistent across devices — PowerShell and Bash installers driven by a single plugins.json manifest.' },
...
const it: Translations = {
  ...
  projects: {
    ...
    items: [
      ...
      { type: 'Progetto Personale', status: 'oss', title: 'Riptune — Client Musicale da Terminale', desc: 'Client musicale nativo da terminale per server compatibili Subsonic (Navidrome, Airsonic, Gonic), scritto in Rust per il compositor Wayland niri — controllo D-Bus via MPRIS2, libreria cache SQLite per avvio istantaneo, e copertine inline via grafica Kitty/Sixel.' },
      ...
      { type: 'Progetto Personale', status: 'oss', title: 're-soulcommand', desc: 'Downloader di raccomandazioni musicali che integra playlist ListenBrainz e Last.fm con Navidrome tramite slskd (demone Soulseek). Fork ricostruito attorno a un backend REST API — ripulito dal superfluo, nativo per Docker.' },
      ...
      { type: 'Progetto Personale', status: 'oss', statusExtra: ' · ★ 26', title: 'ObsidianSetup', desc: 'Script di utilità multipiattaforma che rende l’installazione di Obsidian e dei suoi plugin coerente su tutti i dispositivi — installer PowerShell e Bash guidati da un unico manifest plugins.json.' },
...
const de: Translations = {
  ...
  projects: {
    ...
    items: [
      ...
      { type: 'Persönliches Projekt', status: 'oss', title: 'Riptune — Terminal-Musik-Client', desc: 'Terminal-nativer Musik-Client für Subsonic-kompatible Server (Navidrome, Airsonic, Gonic), gebaut in Rust für den niri-Wayland-Compositor — MPRIS2-D-Bus-Steuerung, SQLite-Cache-Bibliothek für sofortigen Start und Album-Cover inline via Kitty/Sixel-Grafik.' },
      ...
      { type: 'Persönliches Projekt', status: 'oss', title: 're-soulcommand', desc: 'Musikempfehlungs-Downloader, der ListenBrainz- und Last.fm-Playlists über slskd (Soulseek-Daemon) mit Navidrome verbindet. Fork, neu aufgebaut um ein REST-API-Backend — von Ballast befreit, Docker-nativ.' },
      ...
      { type: 'Persönliches Projekt', status: 'oss', statusExtra: ' · ★ 26', title: 'ObsidianSetup', desc: 'Plattformübergreifendes Utility-Skript, das die Installation von Obsidian und seinen Plugins geräteübergreifend konsistent macht — PowerShell- und Bash-Installer, gesteuert von einem einzigen plugins.json-Manifest.' },
...
const fr: Translations = {
  ...
  projects: {
    ...
    items: [
      ...
      { type: 'Projet Personnel', status: 'oss', title: 'Riptune — Client Musical en Terminal', desc: 'Client musical natif en terminal pour serveurs compatibles Subsonic (Navidrome, Airsonic, Gonic), construit en Rust pour le compositeur Wayland niri — contrôle D-Bus via MPRIS2, bibliothèque mise en cache SQLite pour un démarrage instantané, et pochettes d’album intégrées via le graphisme Kitty/Sixel.' },
      ...
      { type: 'Projet Personnel', status: 'oss', title: 're-soulcommand', desc: 'Téléchargeur de recommandations musicales intégrant les playlists ListenBrainz et Last.fm à Navidrome via slskd (démon Soulseek). Fork reconstruit autour d’un backend REST API — allégé du superflu, natif Docker.' },
      ...
      { type: 'Projet Personnel', status: 'oss', statusExtra: ' · ★ 26', title: 'ObsidianSetup', desc: 'Script utilitaire multiplateforme rendant l’installation d’Obsidian et de ses extensions cohérente sur tous les appareils — installateurs PowerShell et Bash pilotés par un manifeste plugins.json unique.' },
